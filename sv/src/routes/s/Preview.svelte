<script lang="ts">
  ...
  function rgb_to_rgb(r: number, g: number, b: number) {
        ...
        >
          {#if selection_outline_enabled}
            <div
             ...
             "
            >
              {JSON.stringify(startCoords)} to {JSON.stringify(endCoords)}
            </div>
          {/if}
...
<style>
  ...
  .c0 {
    /* a flex box the size of the container of arbitrary space */
    width: 100%;
    height: 100%;
    display: flex;
  ...
  .c1 {
    /* a flex box constraining the height, but potentially narrower width */
    max-width: 100%;
    height: 100%;
    display: flex;
    flex-shrink: 1;
  ...
  .c2 {
    /* a flex box constraining the width, but potentially shorter height */
    max-height: 100%;
    width: 100%;
    flex-shrink: 1;
    flex-direction: row;
